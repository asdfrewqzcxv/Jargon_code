...
def filter_redundant(feature, X_arr, nodes, succ_node_to_nodes, pred_node_to_nodes):
  if len(feature.absList) < 3:
    ...
    #elif root = 1:
    elif feature.root == 0:#root = 2
      filtered_nodes = set()
      for _, val in enumerate(nodes):
        if exist_path0(feature, X_arr, val, succ_node_to_nodes, pred_node_to_nodes):
          filtered_nodes.add(val)
      return filtered_nodes
    elif feature.root == 2:
      filtered_nodes = set()
      for _, val in enumerate(nodes):
        if exist_path2(feature, X_arr, val, succ_node_to_nodes, pred_node_to_nodes):
          filtered_nodes.add(val)
      return filtered_nodes
  else:
...
def exist_path0(feature, X_arr, val,succ_node_to_nodes, pred_node_to_nodes):
  succ_nodes = succ_node_to_nodes[val]
  succ_nodes = process_abstract_node(feature.absList[1], succ_nodes, X_arr)

  succ_succ_nodes = set() 
  for _, node in enumerate(succ_nodes):
    succ_succ_nodes = succ_succ_nodes | succ_node_to_nodes[node]
  succ_succ_nodes = process_abstract_node(feature.absList[2], succ_succ_nodes, X_arr)
  #if val == 248:
  #  print("SSSSS")
  #  print(succ_nodes)
  #  print(succ_succ_nodes)
  #  print(len(succ_succ_nodes - set([val])) > 1)
  #Minseok
...
def exist_path2(feature, X_arr, val,succ_node_to_nodes, pred_node_to_nodes):
  pred_nodes = pred_node_to_nodes[val]
  pred_nodes = process_abstract_node(feature.absList[1], pred_nodes, X_arr)

  pred_pred_nodes = set() 
  for _, node in enumerate(pred_nodes):
    pred_pred_nodes = pred_pred_nodes | pred_node_to_nodes[node]
  pred_pred_nodes = process_abstract_node(feature.absList[0], pred_pred_nodes, X_arr)
  #Minseok 
